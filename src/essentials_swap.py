...
from whirlpool_essentials.static_client.accounts import TickArray
from whirlpool_essentials.quote import QuoteBuilder, SwapQuote, SwapQuoteParams
from whirlpool_essentials.quote.swap.swap_manager import PublicKeyWithTickArray
from whirlpool_essentials.types import Percentage
from whirlpool_essentials.quote.swap.swap_manager import SwapDirection, SpecifiedAmount
...
async def get_tick_arrays(ctx: WhirlpoolContext, pubkeys: List[PublicKey]) -> List[PublicKeyWithTickArray]:
    fetched = await ctx.fetcher.list_tick_arrays(pubkeys, True)
    return [PublicKeyWithTickArray(pubkey, data) for pubkey, data in zip(pubkeys, fetched)]
